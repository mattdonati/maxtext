...
mkdir -p /deps/src/src/dependencies/scripts && ( [ -f /deps/src/src/dependencies/scripts/setup_gcsfuse.sh ] || ln -sf /deps/src/dependencies/scripts/setup_gcsfuse.sh /deps/src/src/dependencies/scripts/setup_gcsfuse.sh ); \
mkdir -p /deps/src/src/maxtext && ( [ -d /deps/src/src/maxtext/configs ] || ln -sf /deps/src/maxtext/configs /deps/src/src/maxtext/configs ); \
if [[ \"\${MOUNT_GCSFUSE:-1}\" = \"1\" ]]; then \
  bucket=\"\${GCS_BASE#gs://}\"; bucket=\"\${bucket%/}\"; \
  bash /deps/src/dependencies/scripts/setup_gcsfuse.sh DATASET_GCS_BUCKET=\"\${bucket}\" MOUNT_PATH=\"\${LOCAL_MOUNT}\"; \
fi; \
export PYTHONPATH=/app/src:\${PYTHONPATH:-}; \
cd /app/src && \
unset XLA_FLAGS; \
export LIBTPU_INIT_ARGS=\"${LIBTPU_INIT_ARGS}\"; \
LOAD_PARAM_ARG=\"\"; if [ -n \"\${LOAD_PARAMETERS_PATH:-}\" ]; then LOAD_PARAM_ARG=\"load_parameters_path=\${LOAD_PARAMETERS_PATH}\"; fi; \
python3 -m maxtext.trainers.pre_train.train \
  ...
  pure_nnx=true \
  dcn_diloco_parallelism=\"\${XPK_NUM_SLICES}\" \
...
export OUTPUT_DIR='${BASE_OUTPUT_DIRECTORY}'; \
...
export ADAM_B1='${ADAM_B1}'; \
export ADAM_B2='${ADAM_B2}'; \
export ADAM_EPS='${ADAM_EPS}'; \
export ADAM_WD='${ADAM_WD}'; \
export GRAD_CLIP='${GRAD_CLIP}'; \
export Z_LOSS='${Z_LOSS}'; \
export LOAD_BALANCE_LOSS_WEIGHT='${LOAD_BALANCE_LOSS_WEIGHT}'; \
export FLOAT32_GATE_LOGITS='${FLOAT32_GATE_LOGITS}'; \
export ENABLE_STREAMING_DILOCO='${ENABLE_STREAMING_DILOCO}'; \
export DILOCO_SYNC_PERIOD='${DILOCO_SYNC_PERIOD}'; \
export DILOCO_OUTER_LR='${DILOCO_OUTER_LR}'; \
export DILOCO_OUTER_MOMENTUM='${DILOCO_OUTER_MOMENTUM}'; \
export DILOCO_NUM_FRAGMENTS='${DILOCO_NUM_FRAGMENTS}'; \
export DILOCO_USE_SEQUENTIAL_LAYERS='${DILOCO_USE_SEQUENTIAL_LAYERS}'; \
export DATA_SEED='${DATA_SEED}'; \
export MOUNT_GCSFUSE=1; \
export XPK_NUM_SLICES='${XPK_NUM_SLICES}'; \
export PER_DEVICE_BATCH_SIZE='${PER_DEVICE_BATCH_SIZE}'; \
export WARMUP_FRAC='${WARMUP_FRAC}'; \
${CMD}"
