...
class TinyDecoder(nnx.Module):
  ...
  def __init__(
      ...
      hidden: int,
      rngs: nnx.Rngs,
      cfg: pyconfig.HyperParameters = None,
  ):
    self.mesh = get_tpu_mesh()
    if cfg is None:
      cfg = mock.MagicMock(spec=pyconfig.HyperParameters)
      cfg.emb_dim = hidden
      cfg.mlp_dim = hidden * 2
      cfg.shard_mode = 0
...
def setup_mock_config(
  ...
  """Builds a mock or real pyconfig.HyperParameters instance for testing."""
  if model_name != "simple_mlp":
    ...
    if config_cls == types.MaxTextConfig:
      argv.extend(["enable_tensorboard=False", "record_internal_nn_metrics=False"])
    if cli_overrides:
  ...
  cfg = mock.MagicMock(spec=pyconfig.HyperParameters)
  cfg.init_weights_seed = 42
  cfg.model_name = model_name
  cfg.micro_batch_size_to_train_on = 2
  cfg.vocab_size = 8
  cfg.emb_dim = 4
  cfg.mlp_dim = 8
  cfg.shard_mode = 0
  ...
  cfg.learning_rate_schedule = lambda step: 0.01
  cfg.tensorboard_dir = "/tmp/tb_dir"
  cfg.run_name = "parity_test_run"
  cfg.enable_tensorboard = False
  cfg.z_loss_multiplier = 0.0
  ...
  cfg.num_vocab_tiling = 1
  cfg.num_experts = 1
  cfg.routed_bias = False
  cfg.routed_bias_update_rate = 0.0
  cfg.mtp_num_layers = 0
  cfg.mtp_eval_target_module = 0
  cfg.use_qk_clip = False
  cfg.use_tunix_gradient_accumulation = True
  cfg.shard_optimizer_over_data = False
  cfg.optimizer_memory_host_offload = False
  cfg.parameter_memory_host_offload = False
  cfg.gradient_clipping_threshold = gradient_clipping_threshold
  cfg.grad_dtype = jnp.float32
  cfg.record_internal_nn_metrics = False
  cfg.skip_step_on_spikes = skip_step_on_spikes
  cfg.shard_mode = 0
  cfg.debug_sharding = False
  cfg.weight_sparsity_n = 0
  cfg.weight_sparsity_m = 0
  cfg.pure_nnx = True
  cfg.logical_axis_rules = [
      ("activation_batch", "data"),
      ("activation_length", "fsdp"),
      ("activation_embed", "expert"),
  ]
  cfg.input_data_sharding_logical_axes = [
      "activation_batch",
      "activation_length",
  ]
...
def verification_harness(cfg: pyconfig.HyperParameters, compiled: bool = False) -> Any:
  ...
  with (
      mock.patch.object(
          ...
          "from_pretrained",
          return_value=model_e,
      ),
      mock.patch.object(
          maxtext_engine.train_utils,
          "create_training_optimizer",
          return_value=(
              getattr(cfg, "learning_rate_schedule", lambda step: 0.01),
              opt_e,
          ),
      ),
  ):
...
def verify_auxiliary_metrics_and_telemetry_parity(
  ...
  if cfg is None:
    cfg = setup_mock_config(skip_step_on_spikes=True, gradient_clipping_threshold=1.0)
...
def verify_gradient_accumulation_parity(
  ...
  if hasattr(cfg, "_flat_config") and isinstance(getattr(cfg, "_flat_config", None), dict):
    cfg._flat_config["learning_rate_schedule"] = lr_schedule  # pylint: disable=protected-access
    cfg._flat_config["use_tunix_gradient_accumulation"] = True  # pylint: disable=protected-access
  else:
    try:
      cfg.learning_rate_schedule = lr_schedule
      cfg.use_tunix_gradient_accumulation = True
    except Exception:  # pylint: disable=broad-except
      object.__setattr__(cfg, "learning_rate_schedule", lr_schedule)
    ...
    for step in range(num_steps):
      ...
      if ctx.engine.micro_step_count != m_steps:
        raise ParityVerificationError(
            f"Step {step}: Expected micro_step_count={m_steps}, got {ctx.engine.micro_step_count}"
        )
      ...
      if ctx.engine.micro_step_count != 0:
        raise ParityVerificationError(
            f"Step {step}: Expected micro_step_count=0 after update(), got {ctx.engine.micro_step_count}"
        )
...
def run_all_verifications(cli_overrides: list[str] | None = None) -> None:
  ...
  cfg_base = setup_mock_config(
      model_name=model_name,
      gradient_accumulation_steps=1,
      cli_overrides=overrides,
  )
  cfg_aux = setup_mock_config(
      model_name=model_name,
      gradient_accumulation_steps=1,
  ...
  if model_name == "simple_mlp":
    ...
    if test_suite in ("all", "eager_all", "eager", "1"):
      ...
      print("✓ verify_parity_with_train_py (Eager) passed successfully.", flush=True)
    ...
    if test_suite in ("all", "eager_all", "eager", "3", "gradient_accumulation"):
      print(
  ...
  print(
      f"\n=== Running Training Engine Parity Verification Suite (JIT Compiled Mode, test_suite={test_suite}) ===",
      flush=True,
  )
  ...
  if test_suite in ("all", "jit_all", "jit_auxiliary_metrics", "5", "auxiliary", "telemetry"):
    print(
  ...
  if test_suite in ("all", "jit_all", "jit_gradient_accumulation", "6", "gradient_accumulation", "ga"):
    print(
