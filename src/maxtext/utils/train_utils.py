...
from maxtext.utils import diloco_utils
...
def setup_train_loop(config, recorder, devices=None):
  ...
  with maybe_record_goodput(recorder, GoodputEvent.TRAINING_PREPARATION):
    ...
    if isinstance(state, diloco.DiLoCoTrainState):
      ...
      state_mesh_shardings_params = state_mesh_shardings.model.filter(nnx.Param).to_pure_dict()
      ...
      with jax.set_mesh(mesh), nn_partitioning.axis_rules(config.logical_axis_rules):
        if not isinstance(state, diloco.DiLoCoTrainState):
          ...
          outer_opt_state_sharding = (
              optax.TraceState(trace=state_mesh_shardings_params),
              optax.EmptyState(),
          )
