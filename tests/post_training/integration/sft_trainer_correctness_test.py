...
def setup_maxtext_model(config):
  ...
  if config.pure_nnx:
    # NNX model: params live on the module, so there is no separate train state.
    rngs = maxtext_utils_nnx.create_nnx_rngs(config, rng_key=init_rng)
    with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
      maxtext_model = model_creation_utils.from_config(config, mesh=mesh, rngs=rngs)
    return maxtext_model, None, init_rng
...
def get_maxtext_logits(config, maxtext_data):
  ...
  if config.pure_nnx:
    # NNX forward: the model carries its own params and rng state.
    return maxtext_model(
        decoder_input_tokens=maxtext_data["inputs"],
        decoder_positions=maxtext_data["inputs_position"],
