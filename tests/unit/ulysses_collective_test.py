...
and independent Q, K, and V gradients.
"""
...
def _run_collective_checks():
  """Runs the exchange layout, round-trip, attention, and gradient checks."""
  ulysses_size = 4
  batch, num_query_heads, num_kv_heads, seq_len, head_dim = 2, 8, 4, 32, 4
  assert len(jax.devices()) == ulysses_size, jax.devices()
  ...
  # Segments begin and end inside different context shards (shards of 8), with
