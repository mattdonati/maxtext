...
export WORKLOAD_NAME="${WORKLOAD_NAME:-qwen3-06b-diagnose}"
export DOCKER_IMAGE="${DOCKER_IMAGE:-gcr.io/${PROJECT_ID}/maxtext-rl:latest}"
...
# MaxText command
# max concurrency is overall max concurrency 
...
xpk workload create-pathways \
  ...
  --command="pip install git+https://github.com/AI-Hypercomputer/pathways-utils.git --no-deps; python3 scripts/patch_vllm_sampler.py; python3 scripts/patch_tunix_metrics.py; python3 scripts/patch_tunix_batch_size.py; python3 scripts/patch_math_verify_simple.py; (python3 scripts/memory_profiler.py &); ${MAXTEXT_COMMAND}"


#   --pathways-gcs-location="gs://YOUR_BUCKET_NAME/tmp"
